...
create_player_shot_chart <- function(team, player){
  # create shot data for a specific player in available games
  player_shots <- wbb_shots |> 
    filter(team_short_display_name == team, 
           athlete_display_name == player)
  
  # download player headshot if available
  if(!is.na(player_shots$athlete_headshot_href[1])){
    headshot_png <- paste0(str_replace_all(player, " ", ""), "Headshot.png")
  ...
  ggplot(data = player_shots,
    ...
    labs(
      ...
      title = ifelse(!is.na(player_shots$athlete_headshot_href[1]),
                     paste0("<img src = '", html_safe_player_name, "Headshot.png'", " height = 30>",
                            "<span style='font-size: 25pt'>",
                            player,
                            " Shot Chart</span>"),
                     paste0(player, " Shot Chart")
      ),
  ...
  #canvas(h = 5, w = 5, units = "in", dpi = 600)
}
